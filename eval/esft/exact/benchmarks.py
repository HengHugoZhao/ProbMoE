...
class VLLMWrapper:
    ...
    def _truncate_one(self, text: str) -> str:
        # keep prompt <= max_model_len - max_new_tokens
        max_prompt_len = 4096 - self.max_new_tokens
        ids = self.tokenizer.encode(text, add_special_tokens=False)
        if len(ids) <= max_prompt_len:
            return text
        # keep the tail (usually keeps the actual question/instruction)
        ids = ids[-max_prompt_len:]
        return self.tokenizer.decode(ids, skip_special_tokens=True)

    def generate(self, prompts):
        prompts = [self._truncate_one(p) for p in prompts]
...
class IntentEvaluator(BaseEvaluator):
    ...
    def _metrics(self, prediction, ground_truth):
        ...
        try:
            ...
            print(f"unable to get intent_em")
            return 0.0
        try:
...
class TranslationEvaluator(GPT4Evaluator):
    ...
    def post_process(self, raw_prediction, raw_answers):
        ...
        if isinstance(raw_answers, str):
            ground_truth = raw_answers
        elif isinstance(raw_answers, list) and raw_answers:
            ground_truth = raw_answers[0]
        else:
            ground_truth = ""
